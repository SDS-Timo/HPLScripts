...
export type FtTransferErrors =
  | { DeletedVirtualAccount: null }
  | { InvalidArguments: string }
  | { InsufficientFunds: null };
export type GidStatus = { dropped: {} } | { awaited: {} } | { processed: [[] | [TxResult]] };
export type GlobalId = [bigint, bigint];
export type IdRange = [bigint, [] | [bigint]];
export type IdSelector = { id: bigint } | { cat: Array<{ id: bigint } | { idRange: IdRange }> } | { idRange: IdRange };
export interface LedgerIngressAPI {
  ...
  createFungibleToken: ActorMethod<[number, string], { ok: AssetId } | { err: { NoSpace: null } | { FeeError: null } }>;
...
export type RemoteIdRange = [Principal, bigint, [] | [bigint]];
export type RemoteSelector =
  | { id: RemoteId }
  | { cat: Array<{ id: RemoteId } | { idRange: RemoteIdRange }> }
